...
def _gather_neighbors(X, knn):
    ...
    return Xp.gather(1, idx).permute(0, 3, 1, 2)           # (B, D, N, K)


def _gather_mask(mask, knn):
    """mask (B,N) bool, knn (B,N,K) -> neighbour validity (B,1,N,K)."""
    B, N = mask.shape
    M = mask.unsqueeze(1).unsqueeze(2).expand(B, 1, N, N)
    return M.gather(2, knn.unsqueeze(1).expand(B, 1, N, knn.size(2)))
...
class _EdgeConv(nn.Module):
    ...
    def forward(self, X, knn, mask):
        ...
        edge = edge.masked_fill(~nm.unsqueeze(1), 0.0)
...
class DGCNNSeg(nn.Module):
    ...
    def __init__(self, in_dim=7, hidden=64, hidden2=128, depth_head=False,
                 dropout=0.1):
        super().__init__()
        self.ec1 = _EdgeConv(in_dim, hidden, hidden, dropout)
        self.ec2 = _EdgeConv(in_dim + hidden, hidden2, hidden2, dropout)
...
class _RelPosBlock(nn.Module):
    ...
    def forward(self, X, knn, mask):
        ...
        q = self.q(self.norm_q(X))                          # (B, D, N)
        k = self.k(self.norm_k(xn)) + self.pos(rel)         # (B, D, N, K)
        v = self.v(self.norm_v(xn))                         # (B, D, N, K)
        qh = q.view(B, N, self.h, self.dh).permute(0, 2, 1, 3)      # (B,h,N,dh)
        kh = k.view(B, N, K, self.h, self.dh).permute(0, 3, 1, 2, 4)
        vh = v.view(B, N, K, self.h, self.dh).permute(0, 3, 1, 2, 4)
...
class PointTransformerSeg(nn.Module):
    ...
    def forward(self, X, mask, knn):
        ...
        for blk, ln in zip(self.blocks, self.norms):
            h = h + blk(h, knn, mask)
            h = ln(h.permute(0, 2, 1)).permute(0, 1, 2)
